locals {
  name = "spherical-mammoth-${var.environment}-poorman-k3s"
  tags = merge({
...
module "networking" {
  source = "../poorman-aws/modules/networking"

  name                   = local.name
  availability_zone      = var.availability_zone
  vpc_cidr               = var.vpc_cidr
  private_subnet_cidr    = var.private_subnet_cidr
  instance_ingress_ports = [22, 80, 443, 6443]
  tags                   = local.tags
}

module "storage" {
  source = "../poorman-aws/modules/storage"
...
module "volume_attachment" {
  source = "../poorman-aws/modules/volume-attachment"
...
module "operator_access" {
  source = "../poorman-aws/modules/operator-access"
  ...
  deployment_bucket_arn   = module.storage.bucket_arn
  deployment_prefix       = var.deployment_prefix
  allowed_tunnel_ports    = [22, 80, 443, 6443]
